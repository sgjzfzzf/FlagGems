...
def main() -> None:
    ...
    weight = torch.randn(args.size, device="cuda", dtype=torch.float32)
    eps = 1e-5
    normalized_shape = [args.size]
    ref = torch.nn.functional.rms_norm(x, (args.size,), weight=weight, eps=eps)
    fn = build(args.mode)
