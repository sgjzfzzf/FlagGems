#!/usr/bin/env python3
"""Orchestrate multi-shape FlagGems kernel benches.

Per (op, round, mode): fresh subprocess + temp compile cache.

Shapes: gems pool → sort by cost → take min(n_shapes, available).
Each round: FORMAL_MULTI_PASSES timed full passes over all shapes.
Modes: no guard, no cudagraph.
...
_RUNNER = _HERE / "run_multi_shape.py"
...
def run_one(
    *,
    ...
    if proc.returncode != 0:
        ...
        raise RuntimeError(
            f"run_multi_shape.py failed op={op} mode={mode} exit={proc.returncode}\n"
            f"cmd: {' '.join(cmd)}\nstdout:\n{proc.stdout[-4000:]}"
        )
...
def format_log_line(result: dict, *, round_idx: int) -> str:
    parts = [
        ...
        f"n_shapes={result.get('n_shapes')}",
        f"passes={result.get('passes')}",
        f"shapes={result.get('shapes')}",
    ...
    if "shape_indices" in result:
        parts.append(f"shape_indices={result['shape_indices']}")
    if "pass_indices" in result:
        parts.append(f"pass_indices={result['pass_indices']}")
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--passes", type=int, default=FORMAL_MULTI_PASSES)
    ...
    p.add_argument(
        ...
        help="Comma-separated modes (multi: no guard, no cudagraph)",
    )
    p.add_argument(
        "--output-dir",
        type=Path,
        default=_HERE / "bench_results",
        help="Writes bench-multiple.log and <op>-multiple.json",
    )
    ...
    p.add_argument(
        "--force",
        action="store_true",
        help="Re-run even if <op>-multiple.json already exists",
    )
    return p.parse_args()
...
def main() -> None:
    ...
    for mode in modes:
        if mode not in MULTI_MODES:
            raise SystemExit(f"unknown multi mode: {mode}")
        if "cudagraph" in mode or "guard" in mode:
            raise SystemExit(f"guard/cudagraph not supported in multi: {mode}")

    args.output_dir.mkdir(parents=True, exist_ok=True)
    log_path = args.output_dir / "bench-multiple.log"

    header = (
        f"[multiple] ops={ops} modes={len(modes)} rounds={args.rounds} "
        f"n_shapes<={args.n_shapes} passes={args.passes} warmup=0 "
        f"output={args.output_dir} force={args.force}"
    )
    ...
    with log_path.open("a", encoding="utf-8") as log:
        ...
        for op in ops:
            op_json = args.output_dir / f"{op}-multiple.json"
            if op_json.is_file() and op_json.stat().st_size > 0 and not args.force:
                msg = f"skip {op}/multiple (exists: {op_json})"
                print(f"\n=== {msg} ===", flush=True)
                log.write(f"=== {msg} ===\n")
                log.flush()
                continue

            print(f"\n=== op {op} (multiple) ===", flush=True)
            log.write(f"=== op {op} (multiple) ===\n")
            log.flush()
                    ...
                    for mode in modes:
                        ...
                        try:
                            ...
                            record = {
                                ...
                                "shape_indices": result.get("shape_indices"),
                                "pass_indices": result.get("pass_indices"),
                            ...
                            msg = f"{op}/multiple round={rd} mode={mode}: {exc}"
    ...
    if failures:
        print(f"[multiple] {len(failures)} failure(s) (continued)", flush=True)
        for f in failures:
