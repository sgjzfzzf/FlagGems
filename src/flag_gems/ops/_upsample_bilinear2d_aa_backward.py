...
@triton.jit
def _bilinear_aa_filter(x):
    """Triangle (linear) filter, PIL/PyTorch-compatible.  x must be >= 0.

    ``aa_filter(x) = max(1 - x, 0)`` with support radius 1.  This is the
    anti-aliasing filter PyTorch uses for ``_upsample_bilinear2d_aa``.
    """
    return tl.where(x < 1.0, 1.0 - x, 0.0)
...
def _fused_backward_kernel(
    ...
    for d_ow in tl.static_range(MAX_OW):
        ...
        for j_w in tl.static_range(MAX_KSIZE_W):
            ...
            w_w = _bilinear_aa_filter(arg_w)
            total_wx += tl.where(j_w < xsize_w_pos, w_w, 0.0)

        raw_wx = _bilinear_aa_filter(tl.abs((iw_f - center_w + 0.5) * invscale_w))
        ...
        for d_oh in tl.static_range(MAX_OH):
            ...
            for j_h in tl.static_range(MAX_KSIZE_H):
                ...
                w_h = _bilinear_aa_filter(arg_h)
                total_wy += tl.where(j_h < ysize_h_pos, w_h, 0.0)

            raw_wy = _bilinear_aa_filter(tl.abs((ih_f - center_h + 0.5) * invscale_h))
...
def _precompute_weights_kernel(
    ...
    raw_w = tl.where((offs < xsize) & offs_mask, _bilinear_aa_filter(arg), 0.0)
...
def _bilinear_aa_filter_scalar(x):
    x = abs(float(x))
    if x < 1.0:
        return 1.0 - x
    return 0.0
...
def _get_weight_matrix(
    ...
    # Bilinear AA support radius is 1 at unit scale (INTERP_SIZE / 2 == 1).
    support = 1.0 * scale if scale >= 1.0 else 1.0
    ...
    for oi in range(output_size):
        ...
        values = [
            _bilinear_aa_filter_scalar((ii - center + 0.5) * invscale)
...
def _upsample_bilinear2d_aa_backward(
    ...
    logger.debug("GEMS UPSAMPLE_BILINEAR2D_AA_BACKWARD")
    N, C, H_in, W_in = input_size
    H_out, W_out = output_size
    ...
    # Bilinear AA uses a 2-tap triangle filter (INTERP_SIZE == 2).
    INTERP_SIZE = 2
