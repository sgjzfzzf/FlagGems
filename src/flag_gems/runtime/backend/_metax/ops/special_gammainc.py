...
@triton.jit
def gammainc_kernel_metax(a_ptr, x_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr):
    ...
    # Term recurrence: term_0 = 1/a, term_n = term_{n-1} * x / (a + n)
    # Use tl.where to zero out the term once converged, so subsequent
    # multiplications produce 0 and the sum is not affected.
    # 60 iterations is sufficient for float32 (machine eps ~1.2e-7).
    # =========================================================================
    series_sum = 0.0
    term = 1.0 / a_f32
    series_sum = term
    for i in range(1, 60):
        ...
        # Zero out converged lanes to avoid unnecessary accumulation
        converged = tl.abs(term) < tl.abs(series_sum) * 1e-8
        term = tl.where(converged, 0.0, term)
    ...
    #
    # 100 iterations is sufficient for float32 convergence.
    # Once |delta - 1| < eps, freeze delta at 1.0 so f_val stops changing.
    # =========================================================================
    tiny = 1e-30
    b0 = x_f32 + 1.0 - a_f32
    f_val = b0
    ...
    for i_val in range(1, 100):
        ...
        D_val = 1.0 / D_val
        delta = C_val * D_val
        # Freeze converged lanes: once delta ~ 1, multiplying by 1 is a no-op
        cf_converged = tl.abs(delta - 1.0) < 1e-8
        delta = tl.where(cf_converged, 1.0, delta)
        f_val = f_val * delta

    log_gamma_a2 = tl_extra_shim.lgamma(a_f32)
    log_q = a_f32 * tl.log(x_f32) - x_f32 - log_gamma_a2 - tl.log(f_val)
    q_val = tl.exp(log_q)
...
def _launch_gammainc_metax(out: torch.Tensor, a: torch.Tensor, x: torch.Tensor):
    assert (
...
def special_gammainc(a: torch.Tensor, x: torch.Tensor, *, out: torch.Tensor = None):
    logger.debug("GEMS_METAX SPECIAL_GAMMAINC")
    ...
    _launch_gammainc_metax(out, a, x)
    return out
