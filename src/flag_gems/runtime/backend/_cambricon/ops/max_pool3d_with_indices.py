...
@libtuner(
    configs=[
        triton.Config({"BLOCK_H": 8, "BLOCK_W": 8}, num_stages=0, num_warps=1),
    ],
    key=[
        "out_d",
        "out_h",
    ...
    for kd in tl.static_range(0, kernel_d):
        ...
        for kh in tl.static_range(0, kernel_h):
            for kw in tl.static_range(0, kernel_w):
                ...
                current_idx = kd * kernel_h * kernel_w + kh * kernel_w + kw
...
@libtuner(
    configs=[
        triton.Config({"BLOCK_IN_H": 8, "BLOCK_IN_W": 8}, num_stages=0, num_warps=1),
    ],
    key=[
        "in_d",
        "in_h",
    ...
    for kd in tl.static_range(0, kernel_d):
        ...
        for kh in tl.static_range(0, kernel_h):
            for kw in tl.static_range(0, kernel_w):
                ...
                current_window_idx = kd * kernel_h * kernel_w + kh * kernel_w + kw
                match_mask = indices_block == current_window_idx
...
def max_pool3d_with_indices(
    ...
    Indices follow Cambricon/MLU max_pool3d window-offset semantics.
    """
    logger.debug("GEMS_CAMBRICON MAX_POOL3D_WITH_INDICES")
...
def max_pool3d_backward(
    ...
    logger.debug("GEMS_CAMBRICON MAX_POOL3D_BACKWARD")
