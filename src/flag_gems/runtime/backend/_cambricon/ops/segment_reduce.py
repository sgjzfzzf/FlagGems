...
def _get_block_size():
    return _MLU_BLOCK_SIZE


def _get_uniform_kernel_config(inner_size):
    if inner_size == 1:
        return 16, 1
    return 4, 64


def _get_uniform_backward_tile_config(inner_size):
    return 1, 64 if inner_size > 1 else 1
...
def _segment_reduce_uniform_sum_mean_backward_kernel(
    ...
    for block_start in range(block_start, total_numel, step):
        ...
        grad_offsets = (
            outer_idx * segment_count + segment_idx
...
def _segment_reduce_uniform_inner1_forward_kernel(
    ...
    for row_start in range(row_start, total_rows, step):
        ...
        output_offsets = row_start + tl.arange(0, BLOCK_M)
...
def _segment_reduce_uniform_lengths(data, reduce, lengths, axis):
    ...
    if segment_length <= _UNIFORM_KERNEL_MAX_SEGMENT_LENGTH:
        ...
        if inner_size == 1:
            ...
            grid = (min(triton.cdiv(total_rows, block_m), TOTAL_CORE_NUM),)
        ...
        block_m, block_k = _get_uniform_kernel_config(inner_size)
        grid = (
            min(triton.cdiv(total_rows, block_m), TOTAL_CORE_NUM),
...
def _segment_reduce_uniform_sum_mean_backward(data, grad, reduce, lengths, axis):
    ...
    grid = (min(triton.cdiv(data.numel(), block_size), TOTAL_CORE_NUM),)
    with torch_device_fn.device(data.device):
...
def _segment_reduce_uniform_other_backward(
    ...
    inner_size = _prod(data.shape[axis + 1 :])
    total_rows = _prod(lengths.shape)
    block_m, block_k = _get_uniform_backward_tile_config(inner_size)
    block_n = min(_get_block_size(), triton.next_power_of_2(segment_length))
...
def _lengths_to_offsets_kernel(
    ...
    pid = tle.program_id(0)
    num_jobs = tl.num_programs(0)
    for pid in range(pid, outer_count, num_jobs):
...
def _segment_reduce_forward_kernel(
    ...
    for pid in range(pid, total_numel, num_jobs):
        ...
        if IS_MEAN:
            ...
            acc = tl.where(
                (segment_length > 0) & ~acc_is_nan, acc / segment_length, acc
            )
...
def _lengths_to_offsets(lengths):
    ...
    if offsets.numel() > 0:
        with torch_device_fn.device(lengths.device):
            _lengths_to_offsets_kernel[(min(outer_count, TOTAL_CORE_NUM),)](
                lengths,
                offsets,
                outer_count,
                segment_count,
...
def segment_reduce(
    ...
    *,
    ...
    logger.debug("GEMS_CAMBRICON SEGMENT_REDUCE")
    _check_reduce_and_dtype(data, reduce)
    axis = _wrap_axis(axis, data.dim())
    if indices is not None:
        raise RuntimeError(
    ...
    grid = (min(output.numel(), TOTAL_CORE_NUM),)
...
def segment_reduce_out(
    ...
    *,
    ...
    logger.debug("GEMS_CAMBRICON SEGMENT_REDUCE_OUT")
...
def _segment_reduce_backward(
    ...
    *,
    ...
    logger.debug("GEMS_CAMBRICON _SEGMENT_REDUCE_BACKWARD")
    if (
        initial is None
    ...
    data_size_axis = data_contig.shape[axis]
    _, initial_prod_value = _make_initial("prod", initial)
    grid = (min(output_contig.numel(), TOTAL_CORE_NUM),)
...
def _segment_reduce_backward_out(
    ...
    *,
    ...
    logger.debug("GEMS_CAMBRICON _SEGMENT_REDUCE_BACKWARD_OUT")
