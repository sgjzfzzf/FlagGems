...
def mse_loss_backward_kernel(
    ...
    total_blocks = tl.cdiv(M, BLOCK_M)

    for block_id in range(pid, total_blocks, num_jobs):
        offset = block_id * BLOCK_M + tl.arange(0, BLOCK_M)
        mask = offset < M
...
def mse_loss_backward(grad_output, self, target, reduction=1):
    ...
    """
    logger.debug("GEMS_CAMBRICON MSE_LOSS_BACKWARD")
    ...
    grid = (min(triton.cdiv(M, BLOCK_M), TOTAL_CORE_NUM),)
