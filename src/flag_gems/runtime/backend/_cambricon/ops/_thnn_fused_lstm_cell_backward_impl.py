...
def _thnn_fused_lstm_cell_backward_impl(
    ...
    """
    logger.debug("GEMS_CAMBRICON _THNN_FUSED_LSTM_CELL_BACKWARD_IMPL")
    ...
    if workspace.dtype == torch.uint8:
        workspace = workspace.view(torch.float32).reshape(batch_size, 4 * hidden_size)
        workspace = workspace.to(cx.dtype)
