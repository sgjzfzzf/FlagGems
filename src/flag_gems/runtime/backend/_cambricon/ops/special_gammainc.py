...
@triton.jit
def gammainc_kernel(
    a_ptr, x_ptr, out_ptr, n_elements, BLOCK_SIZE: tl.constexpr, M: tl.constexpr
):
    ...
    while pid < M:
        ...
        # Series expansion: P(a, x) = exp(-x) * x^a * sum_n x^n / Gamma(a+n+1)
        series_sum = 0.0
        term = 1.0 / a_f32
        series_sum = term
        ...
        series_result = (
            tl.exp(-x_f32 + a_f32 * tl.log(x_f32) - log_gamma_a) * series_sum
        )
...
def special_gammainc(a: torch.Tensor, x: torch.Tensor, *, out: torch.Tensor = None):
    logger.debug("GEMS_CAMBRICON SPECIAL_GAMMAINC")
    ...
    if out is None:
        out_dtype = a.dtype if a.is_floating_point() else torch.get_default_dtype()
