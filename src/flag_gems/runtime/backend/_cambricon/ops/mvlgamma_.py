...
def mvlgamma_kernel_(
    ...
    # The launch grid is capped on MLU, so each program processes a grid-stride loop.
    while block_start < n_elements:
        ...
        # Compute the constant term: p * (p - 1) / 4 * log(pi)
        ...
        block_start += num_programs * BLOCK_SIZE


def mvlgamma_(*args, **kwargs):
    logger.debug("GEMS_CAMBRICON MVLGAMMA_")
    x = args[0]
    ...
    if not x.is_contiguous():
        ...
        if n_elements == 0:
            return x
        grid = lambda meta: (
            min(triton.cdiv(n_elements, meta["BLOCK_SIZE"]), MAX_GRID_SIZE_X),
        )
