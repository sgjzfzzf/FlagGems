...
def _unique_dim_argsort_rank_kernel(
    ...
    for cur_row in tl.range(row, num_keys, num_jobs):
        mask = candidates < num_keys
        cur = tl.load(keys_ptr + cur_row)
        vals = tl.load(keys_ptr + candidates, mask=mask, other=cur)
        before = ((vals < cur) | ((vals == cur) & (candidates < cur_row))) & mask
        rank = tl.sum(before.to(tl.int32), axis=0)
        tl.store(indices_ptr + rank, cur_row)
...
def _unique_dim_gather_1d_kernel(
    ...
    for off in tl.range(block_start, num_elements, step):
...
def _unique_dim_row_hash_chunk_kernel(
    ...
    pid = ext.program_id(0)
    num_jobs = tl.num_programs(0)
    total_jobs = num_rows * num_chunks
    for job in tl.range(pid, total_jobs, num_jobs):
        row = job // num_chunks
        chunk = job - row * num_chunks
...
def _unique_dim_row_hash_reduce_kernel(
    ...
    for cur_row in tl.range(row, num_rows, num_jobs):
        vals = tl.load(
            chunk_hash_ptr + cur_row * num_chunks + chunks, mask=mask, other=0
        )
        tl.store(row_hash_ptr + cur_row, tl.sum(vals, axis=0))
...
def _unique_dim_row_diff_reduce_kernel(
    ...
    for cur_row in tl.range(row, num_rows, num_jobs):
        vals = tl.load(
            row_chunk_diff_ptr + cur_row * num_chunks + chunks, mask=mask, other=0
        )
        tl.store(is_first_ptr + cur_row, tl.sum(vals, axis=0) != 0)
...
def _unique_dim_row_single_chunk_first_kernel(
    ...
    for cur_pos in tl.range(row, num_rows, num_jobs):
        out = tl.full((), True, dtype=tl.int1)
        if cur_pos != 0:
            cur_row = tl.load(sorted_indices_ptr + cur_pos)
            prev_row = tl.load(sorted_indices_ptr + cur_pos - 1)
            cur = tl.load(flat_ptr + cur_row * row_len + offsets, mask=mask)
            prev = tl.load(flat_ptr + prev_row * row_len + offsets, mask=mask)
            neq = (cur != prev) & mask
            out = tl.sum(neq.to(tl.int32), axis=0) != 0
        tl.store(is_first_ptr + cur_pos, out)
...
def _unique_dim_gather_moved_kernel(
    ...
    total_jobs = num_unique * num_chunks
    for job in tl.range(pid, total_jobs, num_jobs):
        row = job // num_chunks
        chunk = job - row * num_chunks
...
def _unique_dim_inverse_permutation_kernel(
    ...
    block_start = (pid * BLOCK_SIZE).to(tl.int64)
    step = num_jobs * BLOCK_SIZE
    for off in tl.range(block_start, num_rows, step):
        offsets = off + tl.arange(0, BLOCK_SIZE)
        mask = offsets < num_rows
...
def _unique_dim_counts_kernel(
    ...
    block_start = (pid * BLOCK_SIZE).to(tl.int64)
    step = num_jobs * BLOCK_SIZE
    for off in tl.range(block_start, num_unique, step):
        offsets = off + tl.arange(0, BLOCK_SIZE)
...
def _grid_1d(num_jobs: int) -> tuple[int, int, int]:
    return (min(max(num_jobs, 1), TOTAL_CORE_NUM), 1, 1)


def _grid_blocks(num_elements: int, block_size: int) -> tuple[int, int, int]:
    return _grid_1d(triton.cdiv(num_elements, block_size))
...
def _unique_dim_build_key_kernel(
    ...
    for off in tl.range(block_start, num_rows, step):
        ...
        if REMAP_KIND == 0:  # _REMAP_INT
            ...
            bits = (
                tl.load(flat_ptr + base, mask=mask, other=0).to(tl.int64) & 0xFFFFFFFF
            )
...
def _build_composite_key(
    ...
    grid = _grid_blocks(num_rows, block_size)
...
def _triton_gather_1d(values: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    ...
    if num_elements == 0:
        return output
    grid = _grid_blocks(num_elements, _UNIQUE_DIM_GATHER_BLOCK_SIZE)
...
def _argsort_keys(keys: torch.Tensor):
    ...
    if num_keys <= _UNIQUE_DIM_RANK_SORT_MAX_KEYS:
        ...
        with torch_device_fn.device(keys.device.index):
            _unique_dim_argsort_rank_kernel[_grid_1d(num_keys)](
                keys.contiguous(),
                perm,
                sorted_keys,
                num_keys,
...
def _unique_dim_row_hash(flat: torch.Tensor) -> torch.Tensor:
    ...
    with torch_device_fn.device(flat.device.index):
        _unique_dim_row_hash_chunk_kernel[_grid_1d(num_rows * num_chunks)](
            flat,
            chunk_hash,
            num_rows,
            row_len,
        ...
        _unique_dim_row_hash_reduce_kernel[_grid_1d(num_rows)](
            chunk_hash,
            row_hash,
            num_rows,
            num_chunks,
...
def _unique_dim_first_mask(flat: torch.Tensor, sorted_indices: torch.Tensor):
    ...
    if num_chunks == 1:
        with torch_device_fn.device(flat.device.index):
            _unique_dim_row_single_chunk_first_kernel[_grid_1d(num_rows)](
                flat,
                sorted_indices,
                is_first,
                num_rows,
    ...
    grid = _grid_1d(num_rows * num_chunks)
    row_hash = (
    ...
    with torch_device_fn.device(flat.device.index):
        ...
        _unique_dim_row_diff_reduce_kernel[_grid_1d(num_rows)](
            row_chunk_diff,
            is_first,
            num_rows,
            num_chunks,
...
def _unique_dim_gather_output(
    ...
    grid = _grid_1d(num_unique * num_chunks)
...
def _unique_dim_inverse_from_permutation(sorted_indices: torch.Tensor) -> torch.Tensor:
    ...
    grid = _grid_blocks(num_rows, _UNIQUE_DIM_GATHER_BLOCK_SIZE)
...
def _unique_dim_counts(
    ...
    grid = _grid_blocks(num_unique, _UNIQUE_DIM_GATHER_BLOCK_SIZE)
...
def unique_dim(
    ...
    """
    logger.debug("GEMS_CAMBRICON UNIQUE_DIM")
