...
def _unsupported_svd(input, some=True, compute_uv=True, reason=None):
    ...
    raise NotImplementedError(
        "FlagGems native SVD currently supports float32 mlu matrices with "
...
def _is_float32_mlu_matrix(input):
    return input.is_mlu and input.dtype == torch.float32 and input.dim() >= 2


def _is_low_precision_mlu_matrix(input):
    return (
        input.is_mlu
...
def _can_use_rank1_kernel(input, some=True, compute_uv=True):
    ...
    return _is_float32_mlu_matrix(input) and some and compute_uv and min(m, n) == 1


def _can_use_rank2_kernel(input, some=True, compute_uv=True):
    _, m, n = _svd_shape(input)
    return (
        _is_float32_mlu_matrix(input)
        and some
        and compute_uv
        and min(m, n) == 2
...
def _can_use_4x4_kernel(input, some=True, compute_uv=True):
    _, m, n = _svd_shape(input)
    return _is_float32_mlu_matrix(input) and some and compute_uv and m == 4 and n == 4
...
def _can_use_gram_kernel(input, some=True, compute_uv=True):
    _, m, n = _svd_shape(input)
    return _is_float32_mlu_matrix(input) and some and compute_uv and min(m, n) <= 1024
...
def svd(input, some=True, compute_uv=True):
    ...
    if _is_low_precision_mlu_matrix(input):
        return SVDResult(*_low_precision_svd_via_float32(input, some, compute_uv))
    if _is_float32_mlu_matrix(input) and 0 in input.shape[-2:]:
    ...
    if not _is_float32_mlu_matrix(input) or not some:
