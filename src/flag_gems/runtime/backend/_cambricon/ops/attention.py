...
def _attn_bwd_dkdv(
    ...
    for blk_idx in range(num_steps):
        ...
        if BWD_KEY_FP32:
            qkT = tl.dot(key, qT.to(tl.float32)).to(tl.float32)
        else:
            qkT = tl.dot(key, qT).to(tl.float32)  # (BLOCK_N1, BLOCK_M1)
        if HAS_ATTN_MASK:
            attn = tl.load(
                attn_mask
                + offs_m[None, :] * stride_attn_mask_q_seqlen
...
def _attn_bwd_dq(
    ...
    for blk_idx in range(num_steps):
        ...
        if BWD_KEY_FP32:
            ...
            qk = tl.dot(query, kT).to(tl.float32)
        if HAS_ATTN_MASK:
            attn = tl.load(
                attn_mask
                + offs_m[:, None] * stride_attn_mask_q_seqlen
...
def _attn_bwd(
    ...
    BWD_KEY_FP32: tl.constexpr = False,
    LN2: tl.constexpr = LN_2,
    RCP_LN2: tl.constexpr = RCP_LN_2,
):
...
def scaled_dot_product_attention_forward(
    ...
    # Use a power-of-2 block size for Triton while masking the real head dim.
...
def scaled_dot_product_attention_backward(
    ...
    BLK_SLICE_FACTOR = 2
    BWD_KEY_FP32 = key.dtype == torch.bfloat16
    if key.dtype == torch.bfloat16:
        bwd_key = key.to(torch.float32) * (sm_scale * RCP_LN_2)
    else:
        bwd_key = key * (sm_scale * RCP_LN_2)
    # PRE_BLOCK = 128
    PRE_BLOCK = 256
    ...
    # Keep KV gradients in fp32 so autograd accumulates without MLU bf16 store
    # rounding spikes in the backward kernel.
    kv_grad_dtype = (
        torch.float32 if key.dtype == torch.bfloat16 or group_head > 1 else key.dtype
    )
