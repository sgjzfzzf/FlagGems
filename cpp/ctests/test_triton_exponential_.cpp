...
TEST(exponential_op_test, exponential_) {
  ...
  // Disabled: the C++ launcher currently loads the generic
  // ops/exponential_.py kernel, which uses tl.philox. On Cambricon MLU,
  // samples generated by this kernel fail the KS distribution check for
  // every lambda.
  //
  // The MLU-specific kernel in runtime/backend/_cambricon/ops/exponential_.py
  // uses the vendor libdevice Philox implementation and avoids this failure:
  // tests/ks_tests.py::test_exponential_pvalue drives the same three lambdas
  // through the Python path and passes on MLU. Before enabling the float32
  // test, update the launcher to prefer the vendor-specific kernel when
  // available. See SORT_KERNEL_PATH in lib/sort.cpp for an example of
  // vendor-kernel selection.
  //
  // TODO: implement vendor-specific exponential kernel selection.
  // RunExponentialTest<float>(torch::kFloat32);

  // Disabled independently of float32: this test requires Triton double
  // support, which is unavailable on some backends, including Cambricon MLU.
